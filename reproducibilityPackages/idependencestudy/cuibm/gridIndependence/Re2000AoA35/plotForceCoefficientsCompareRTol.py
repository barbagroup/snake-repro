...
h=0.006 in the uniform region and relative tolerance of 1.0E-06 for the 
iterative solvers and compares with the solution obtained on the same grid with
relative tolerance of 1.0E-05.
"""
...
simulation_directory = os.path.join(os.path.dirname(__file__), 
                                    'h0.006_rtol6')
simulation = CuIBMSimulation(description='rtol=1.0E-06',
                             directory=simulation_directory)
simulation.read_forces()
simulation.get_mean_forces(limits=[32.0, 64.0])

# Computes the mean force coefficients from the cuIBM simulation
# with grid-spacing h=0.006 in the uniform region and rtol=1.0E-05.
# The force coefficients are averaged between 32 and 64 time-units.
simulation_directory = os.path.join(os.path.dirname(__file__), 
                                    'h0.006')
simulation2 = CuIBMSimulation(description='rtol=1.0E-05',
                              directory=simulation_directory)
